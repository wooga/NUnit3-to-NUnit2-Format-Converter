...
def convert(input_path, output_path, build_target):
 ''' Converts the input file from the NUnit 3 format to NUnit 2 format
     and writes the result into output_path.
     And prepends the buildtarget (iOS,Android,etc.) to each test it's name for easy identifying.
 '''
 ...
 def write_output(results, output_path, build_target):
     ...
     for curr_test_results in test_results:
         ...
         test_case.setAttribute('name', build_target + " "+ curr_test_results['name'])
 ...
 write_output(results, output_path, build_target)
...
def main():
 ...
 required_named.add_argument('-t', '--buildTarget', dest='target', help='Name of the buildTarget', required=True)
 ...
 print ("Converting " + input_path + " to " + output_path + "for build target " + build_target)
 convert(input_path, output_path, build_target)
 return 0
